...
export default function PaymentsView({ activeApp }) {
  ...
  const { visibleCommonRoles, showPrivateParties, visibleCondoIndex } =
    useMemo(() => {
      switch (
    ...
    switch (status) {
      ...
      case WALLET_STATUS.PENDING:
        return (
          <Badge
            variant="outline"
            className="bg-amber-50 text-amber-700 border-amber-100 text-[10px] font-bold"
          >
            IN ATTESA
          </Badge>
        );
      case WALLET_STATUS.TO_ACTIVATE:
        return (
          <Badge
            variant="outline"
            className="bg-slate-50 text-slate-500 border-slate-100 text-[10px] font-bold"
          >
            DA ATTIVARE
          </Badge>
        );
  ...
  const getInvoicesForRole = (roleIdentifier, limit = 100) => {
    ...
    return paymentData.invoiceDetails
      .filter((inv) => inv.to === roleIdentifier)
      .slice(0, limit);
  };
  ...
  if (user?.role === ROLES.CONDOMINO) {
    ...
    const walletStatus =
      paymentData.wallets.condomino[visibleCondoIndex] || WALLET_STATUS.ACTIVE; // Assuming condoWallets is an array
    // For Condomino, we simulate a personal balance. In a real app, this would be specific to the user's unit.
    const personalBalance = 1250.0;
  ...
  if (user?.role === ROLES.AMMINISTRATORE) {
    ...
    const adminWalletStatus =
      paymentData.wallets["amministratore"] || WALLET_STATUS.ACTIVE;
  ...
  const totalToPay = paymentData.invoiceDetails
        ...
        {visibleCommonRoles.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center gap-2 px-1">
              ...
              <h3 className="text-[11px] font-black tracking-[0.2em] text-primary/60 uppercase">
                Parti Comuni
              </h3>
            </div>
            ...
            <Accordion type="single" collapsible className="space-y-3">
              {COMMON_ROLES.filter((r) =>
                visibleCommonRoles.includes(r.id),
              ).map((role) => {
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow className="bg-slate-50/50 hover:bg-slate-50/50">
                              ...
                              <TableHead className="text-right text-[10px] font-bold uppercase h-9">
                                Importo
                              </TableHead>
                              <TableHead className="text-right text-[10px] font-bold uppercase h-9">
                                Stato
                          ...
                          <TableBody>
                            {getInvoicesForRole(role.id).map((inv) => (
                              <TableRow
                                key={inv.id}
                                className="h-10 border-b border-border/20 last:border-0 hover:bg-transparent"
                              >
          ...
          <div className="space-y-4">
            ...
            <Accordion type="multiple" className="space-y-3">
              {condoWallets.map((status, idx) => {
                ...
                if (visibleCondoIndex !== null && idx !== visibleCondoIndex)
                  return null;
