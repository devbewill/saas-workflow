...
export default function GestoreView() {
  ...
  const filtered = PAYMENT_PROJECTS.filter(
    (p) =>
  ...
  const totalInvoiced = PAYMENT_PROJECTS.reduce(
    (sum, p) => sum + p.totalInvoices,
    0,
  );
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard
          label="Volume Transato"
          value={formatCurrency(totalInvoiced)}
          icon={Landmark}
        />
        <StatCard label="Wallet Attivi" value={totalWallets} icon={Wallet} />
        <StatCard
          label="Progetti Gestiti"
          value={PAYMENT_PROJECTS.length}
          icon={ArrowUpDown}
        />
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-slate-50/50 hover:bg-slate-50/50">
              <TableHead className="w-[250px] font-bold uppercase text-[11px] tracking-wider text-primary/60">
                Progetto
              </TableHead>
              <TableHead className="text-center font-bold uppercase text-[11px] tracking-wider text-primary/60">
                Amministratore
              </TableHead>
              <TableHead className="text-center font-bold uppercase text-[11px] tracking-wider text-primary/60">
                Impresa
              ...
              <TableHead className="text-right font-bold uppercase text-[11px] tracking-wider text-primary/60">
                Totale Fatture
              </TableHead>
              <TableHead className="text-right font-bold uppercase text-[11px] tracking-wider text-primary/60">
                Aggiornato
                ...
                {[
                  ...
                  <TableCell key={role} className="text-center">
                    <div className="flex flex-col items-center justify-center gap-1">
                      {role === "condomino" &&
                      Array.isArray(project.wallets[role]) ? (
