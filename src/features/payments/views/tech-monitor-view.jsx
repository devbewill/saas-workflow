...
export default function TechMonitorView() {
  ...
  const allWallets = PAYMENT_PROJECTS.flatMap((p) => {
    ...
    wallets.push({
      id: `WLT-${p.id}-ADM`,
      type: "Amministratore",
      status: p.wallets.amministratore,
      project: p.id,
    });
    wallets.push({
      id: `WLT-${p.id}-IMP`,
      type: "Impresa",
      status: p.wallets.impresa,
      project: p.id,
    });
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Panoramica tecnica dei wallet e servizi"
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <StatCard
          label="Wallet Attivi"
          value={
            allWallets.filter((w) => w.status === WALLET_STATUS.ACTIVE).length
          }
          icon={Wallet}
        />
        <StatCard
          ...
          value={
            allWallets.filter((w) => w.status === WALLET_STATUS.PENDING).length
          }
          icon={Activity}
        />
        <StatCard
          label="Da Attivare"
          value={
            allWallets.filter((w) => w.status === WALLET_STATUS.TO_ACTIVATE)
              .length
          }
          icon={AlertTriangle}
        />
              ...
              <TableRow key={w.id}>
                ...
                <TableCell>
                  <StatusBadge status={w.status}>
                    {w.status ? w.status.replace("_", " ") : "N/A"}
                  </StatusBadge>
                </TableCell>
        ...
        <SectionPanel title="Log Critici" icon={AlertTriangle}>
          <div className="space-y-3">
            {[
              {
                time: "14:30",
                msg: "Wallet WLT-PRJ-004-ADM: Attivazione fallita — KYC incompleto",
                severity: "error",
              },
              {
                time: "12:15",
                msg: "Wallet WLT-PRJ-002-IMP: Timeout connessione provider pagamento",
                severity: "warning",
              },
              {
                time: "09:00",
                msg: "Wallet WLT-PRJ-001-ADM: Ricarica completata con successo",
                severity: "info",
              },
            ].map((log, i) => (
              <div
            ...
            <div className="space-y-3 font-mono text-[10px] opacity-80">
              <div className="flex gap-2">
                ...
                <p className="text-emerald-500">
                  WLT-PRJ-001-ADM: Payout recognized (€2.450)
                </p>
              </div>
              <div className="flex gap-2">
              ...
              <div className="flex gap-2">
                ...
                <p className="text-amber-500">
                  System: Retry webhook WLT-PRJ-009...
                </p>
              </div>
              <div className="flex gap-2">
                <span className="text-slate-500">[14:32:15]</span>
                <p className="text-blue-400">
                  WLT-PRJ-001-CST: Status changed → ACTIVE
                </p>
              </div>
            </div>
          </div>
